...
internal fun ReadableMap?.getOptionalString(key: String): String? =
  if (hasKey(key)) {
    this?.getString(key)
  } else {
    null
...
internal fun ReadableMap?.getOptionalBoolean(key: String): Boolean? =
  if (hasKey(key)) {
    this?.getBoolean(key)
  } else {
    null
...
internal fun ReadableMap?.getOptionalInt(key: String): Int? =
  if (hasKey(key)) {
    this?.getInt(key)
  } else {
    null
...
internal fun ReadableMap?.getOptionalDouble(key: String): Double? =
  if (hasKey(key)) {
    this?.getDouble(key)
  } else {
    null
...
internal fun ReadableMap?.getOptionalMap(key: String): ReadableMap? =
  if (hasKey(key)) {
    this?.getMap(key)
  } else {
    null
...
internal fun ReadableMap?.getOptionalArray(key: String): ReadableArray? =
  if (hasKey(key)) {
    this?.getArray(key)
  } else {
    null
...
internal val ReadableMap.recommendationConfiguration: ReadableMap?
  get() = configuration?.getOptionalMap("recommendationConfiguration")
...
internal val ReadableMap.shouldAskForCSC: Boolean
  get() = uiConfiguration?.getOptionalBoolean("shouldAskForCSC") ?: false

internal val ReadableMap.shouldAskForCardholderName: Boolean
  get() = uiConfiguration?.getOptionalBoolean("shouldAskForCardholderName") ?: false

internal val ReadableMap.recommendationURL: String?
  get() = recommendationConfiguration?.getOptionalString("url")

internal val ReadableMap.recommendationRSAPublicKey: String?
  get() = recommendationConfiguration?.getOptionalString("rsaPublicKey")

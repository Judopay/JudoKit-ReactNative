import JudoPay, {
    JudoAuthorization,
    JudoConfiguration, JudoResponse,
    JudoTransactionMode,
    JudoTransactionType,
    JudoReference
} from 'judokit-react-native'
import {Alert} from "react-native";
import {AlertFunctionProps, DemoFeatureType, Screen} from "../Data/TypeDefinitions";
...
const promiseForFeature = ({ featureType, judo, configuration }: PromiseForFeature) => {
    ...
    switch (featureType) {
        ...
        case DemoFeatureType.CREATE_CARD_TOKEN:
            return judo.invokeTransaction(JudoTransactionType.RegisterCard, configuration)

        case DemoFeatureType.SAVE_CARD:
            return judo.invokeTransaction(JudoTransactionType.SaveCard, configuration)

        case DemoFeatureType.CHECK_CARD:
            return judo.invokeTransaction(JudoTransactionType.CheckCard, configuration)
        ...
        case DemoFeatureType.PAYMENT_METHODS:
            return judo.invokePaymentMethodScreen(JudoTransactionMode.Payment, configuration)

        case DemoFeatureType.PRE_AUTH_METHODS:
            return judo.invokePaymentMethodScreen(JudoTransactionMode.PreAuth, configuration)

        case DemoFeatureType.SERVER_TO_SERVER:
            return judo.invokePaymentMethodScreen(JudoTransactionMode.ServerToServer, configuration)

        default:
            return Promise.reject(new Error(`No Promise for DemoFeatureType: ${featureType} to be returned.`))
    }
}
...
const screenForFeature = (featureType: DemoFeatureType) => {
    switch (featureType) {
        ...
        case DemoFeatureType.PAY_BY_BANK_APP:
            return Screen.PAY_BY_BANK_APP;
...
export const dispatch = ({ featureType, isSandboxed, authorization, configuration, onError, onSuccess, onNavigate }: DispatchProps) => {
...
export const alert = ({ title = "Error", message, onPressOK }: AlertFunctionProps) => {
    Alert.alert(
        title,
        message,
        [
...
export function transformToListOfResultItems(result: JudoResponse): Array<ResultItem> {
    const mapToResultItemList = (objectToMap: Record<string, any>): Array<ResultItem> => {
...
export const regeneratePaymentReferenceIfNeeded = (configuration: JudoConfiguration): JudoConfiguration => {
    ...
    return {
        ...configuration,
        reference: {
            paymentReference: paymentReference?.length === 0 ? Math.random().toString(36).substr(2, 10) : paymentReference,
            consumerReference: consumerReference?.length === 0 ? "my-unique-consumer-ref" : consumerReference
        }
    }
}

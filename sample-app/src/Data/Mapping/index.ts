...
import {
    JudoAddressFormat, JudoAuthorization,
...
export const judoConfigurationFromSettingsData = ({
    ...
    if (connectTimeout?.length > 0 && writeTimeout?.length > 0 && readTimeout?.length > 0) {
    ...
    // paymentMethods
    const {isCardOn, isApplePayOn, isGooglePayOn, isPayByBankAppOn, isiDealOn} = paymentMethods
    let methods = 0
    ...
    if (isThreeDSUIConfigurationEnabled) {
        ...
        let buttonCustomizations: Partial<Record<JudoThreeDSButtonType, JudoThreeDSButtonCustomization>> = {}
    ...
    const threeDSTwoMaxTimeout = maxTimeout.length > 0 ? Number(maxTimeout) : undefined
    const messageVersion = protocolMessageVersion.length > 0 ? protocolMessageVersion : undefined
    ...
    const reduceCallbackFn = (previousValue: JudoContactField, currentValue: JudoContactField) => {
        if (previousValue) {
    ...
    let requiredBillingContactFields = billingFields.length > 0 ? billingFields.reduce(reduceCallbackFn) : undefined
    let requiredShippingContactFields = shippingFields.length > 0 ? shippingFields.reduce(reduceCallbackFn) : undefined
    ...
    if (billingAddressFields === "MIN" || billingAddressFields === "FULL") {
        billingParameters = {
            addressFormat: billingAddressFields === "MIN" ? JudoAddressFormat.MINIMAL : JudoAddressFormat.FULL,
    ...
    const googlePayConfiguration: JudoGooglePayConfiguration = {
        countryCode,
        environment: isProductionEnvironmentOn ? JudoGooglePayEnvironment.PRODUCTION : JudoGooglePayEnvironment.TEST,

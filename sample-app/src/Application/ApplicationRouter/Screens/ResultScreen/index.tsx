...
const ResultScreen: FC<StackScreenProps<RootStackParamList, Screen.RESULT>> = ({
  route: {
    ...
    const { colors: { background: backgroundColor, card, text, primary } } = useTheme()
    ...
    const renderItem = (
        ...
        return (
            <TouchableHighlight onPress={() => onItemPress(item)} onLongPress={(event) => onLongPress(item)}>
                <View
                    style={{
                        flex: 1,
                        flexDirection: 'row',
                ...
                >
                    ...
                    { subItems ? <Icon name="ios-chevron-forward" size={20} color={primary} /> : null }
                </View>
            </TouchableHighlight>
        )
    }
